...
folder and destroy the Dropbox App Sync folder in Dropbox.
...
Revert any synced config file to its original state, and delete the Dropbox App Sync
folder in Dropbox. This will revert your system at pre-Dropbox App Sync state.
...
When you launch `dbas backup`, here's what it's really doing:

1. `cp ~/.gitconfig ~/Dropbox/Dropbox App Sync/.gitconfig`
1. `rm ~/.gitconfig`
1. `ln -s ~/Dropbox/Dropbox App Sync/.gitconfig ~/.gitconfig`

Now your `git` config is always backup and up to date on all your workstations.

### Restore

When you launch `dbas restore`, here's what it's really doing:

1. `ln -s ~/Dropbox/Dropbox App Sync/.gitconfig ~/.gitconfig`

That's it, you got your `git` config setup on your new workstation.

`dbas` does the same for any supported application.
...
Open a [new issue](https://github.com/lra/dbas/issues).

## How can I tell Dropbox App Sync to sync only one or two application ?

In your home folder, create a file named `.dbas.cfg` and add the application
...
A [sample](.dbas.cfg) of this file is available for download:

```bash
cd
curl -o .dbas.cfg https://raw.github.com/lra/dbas/master/.dbas.cfg
```
Be careful, if you download it like this, Dropbox App Sync will only try to sync SSH and
Adium from now on !

## How can I tell Dropbox App Sync to not sync an application ?
...
curl -o .dbas.cfg https://raw.github.com/lra/dbas/master/.dbas.cfg
```
Be careful, if you download it like this, Dropbox App Sync will ignore SSH and Adium from
now on !
...
So here comes Dropbox App Sync, the little tool that will sync all your application
configs to Dropbox.
